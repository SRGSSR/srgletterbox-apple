...
CREATE_CARTFILE_PRIVATE_CLOSED=@(cat $(CARTFILE_HIDDEN) 2> /dev/null; echo) > $(CARTFILE_PRIVATE); cat $(CARTFILE_PROPRIETARY) >> $(CARTFILE_PRIVATE)
CREATE_CARTFILE_PRIVATE_OPEN=@[ -f $(CARTFILE_HIDDEN) ] && cp $(CARTFILE_HIDDEN) $(CARTFILE_PRIVATE) || true
